...
export function ArsipSuratForm({ arsipSurat, userRole }: ArsipSuratFormProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [fileError, setFileError] = useState<string | null>(null);
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(
    arsipSurat?.tanggal ? new Date(arsipSurat.tanggal) : undefined,
  );
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (file) {
      ...
      if (error) {
        ...
        // Update the input element if possible to keep it in sync,
        // though handleSubmit uses state 'selectedFile' so it's not strictly required
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!pengirimPenerima) {
      toast.error("Pengirim/Penerima harus diisi");
      return;
    }

    // Validate Deskripsi (Opsional) - no check via required

    // Validate Jenis Surat
    const jenisSurat = formData.get("jenisSurat");
    if (!jenisSurat) {
      toast.error("Jenis Surat harus dipilih");
      return;
    }
    ...
    if (!perihal) {
      toast.error("Perihal harus diisi");
      return;
    }

    // Validate file if it's a new upload (Optional now)
    if (selectedFile) {
    ...
    try {
      // FormData already created above for validation

      // Add selected date to FormData

      // Add selected date to FormData
      formData.set("tanggal", selectedDate.toISOString());

      // Manually add file if selected via state (since input might be hidden/removed)
      if (selectedFile) {
      ...
      let result;
      if (arsipSurat) {
        result = await updateArsipSurat(arsipSurat.id, formData);
      } else {
        result = await createArsipSurat(formData);
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="noSurat"
                  name="noSurat"
                  defaultValue={arsipSurat?.noSurat}
              ...
              {/* Pengirim/Penerima */}
              <div className="space-y-2">
                <Label htmlFor="pengirimPenerima">
                  Pengirim/Penerima <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="pengirimPenerima"
                  name="pengirimPenerima"
                  defaultValue={
                    arsipSurat?.pengirimPenerima
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="deskripsi"
                  name="deskripsi"
                  defaultValue={arsipSurat?.deskripsi || ""}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select name="jenisSurat" defaultValue={arsipSurat?.jenisSurat}>
                  <SelectTrigger className="w-full">
              ...
              {/* File Upload */}
              <div className="space-y-2">
                <Label htmlFor="file">File Surat</Label>
                <div
                    ...
                    <div className="text-center">
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          (JPG/PNG/WebP). File akan dienkripsi otomatis.
                        </p>
                      </label>
                    </div>
                  ) : (
